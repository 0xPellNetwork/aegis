...
PELLCHAIN_OPENAPI="pellchain.swagger.yaml"
PELLCHAIN_PROTO_PATH="proto/"
APP_GO="app/app.go"
OPENAPI_HEADER="swagger: '2.0'
info:
  title: PellChain - gRPC Gateway docs
  description: Documentation for the API of PellChain."
...
buf generate --template $PROTO_TEMPLATE --output=$OUTPUT_DIR $PELLCHAIN_PROTO_PATH

# Initialize the merged swagger file with header info
echo "$OPENAPI_HEADER" > $OUTPUT_DIR/$MERGED_SWAGGER_FILE

# Extract paths and definitions from the cosmos swagger file and PellChain swagger file
# Merge them using yq and write to the merged swagger file
{
  echo "paths:"
  yq ea 'select(fileIndex == 0).paths * select(fileIndex == 1).paths * select(fileIndex == 2).paths' $COSMOS_OPENAPI_PATH $OUTPUT_DIR/$PELLCHAIN_OPENAPI $ETHERMINT_OPENAPI_PATH | sed -e 's/^/  /'
  echo "definitions:"
  yq ea 'select(fileIndex == 0).definitions * select(fileIndex == 1).definitions * select(fileIndex == 2).definitions' $COSMOS_OPENAPI_PATH $OUTPUT_DIR/$PELLCHAIN_OPENAPI $ETHERMINT_OPENAPI_PATH | sed -e 's/^/  /'
...
if [ -f $OUTPUT_DIR/$MERGED_SWAGGER_FILE ]; then
  # Remove the PELLCHAIN_OPENAPI file
  rm $OUTPUT_DIR/$PELLCHAIN_OPENAPI
fi
