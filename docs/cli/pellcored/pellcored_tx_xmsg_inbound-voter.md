# tx xmsg inbound-voter

Broadcast message sendVoter

### Synopsis

Broadcast message sendVoter. when tx-type is staker-deposited, 
			the pellData include "staker token strategy amount". when tx-type is 
			staker-delegated, the pellData include "staker operator"

```
pellcored tx xmsg inbound-voter [sender] [senderChainID] [txOrigin] [receiver] [receiverChainID] [inTxHash] [inBlockHeight] [eventIndex] [pellData] [flags]
```
...
```
  ...
  -s, --sequence uint            The sequence number of the signing account (offline mode only)
      ...
      --tx-type string           [staker-deposited|staker-delegated] pell data in inbound tx. 
