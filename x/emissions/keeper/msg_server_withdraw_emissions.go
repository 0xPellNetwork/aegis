...
func (k msgServer) WithdrawEmission(goCtx context.Context, msg *types.MsgWithdrawEmission) (*types.MsgWithdrawEmissionResponse, error) {
	...
	err = k.RemoveWithdrawableEmission(ctx, msg.Signer, msg.Amount)
	if err != nil {
		return nil, errorsmod.Wrap(types.ErrUnableToWithdrawEmissions, fmt.Sprintf("error while removing withdrawable emission for address %s : %s", msg.Signer, err))
	}
