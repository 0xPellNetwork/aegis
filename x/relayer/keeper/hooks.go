...
func (k Keeper) CheckAndCleanObserver(ctx context.Context, valAddress sdk.ValAddress) error {
	accAddress, err := types.GetAccAddressFromOperatorAddress(valAddress.String())
	if err != nil {
		return err
	}
...
// if not it removes the observer from the set
func (k Keeper) CheckAndCleanObserverDelegator(ctx context.Context, valAddress sdk.ValAddress, delAddress sdk.AccAddress) error {
	accAddress, err := types.GetAccAddressFromOperatorAddress(valAddress.String())
	if err != nil {
		return err
	}
