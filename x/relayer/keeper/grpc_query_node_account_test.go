...
func TestNodeAccountQuerySingle(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		response *types.QueryNodeAccountResponse
		err      error
	}{
		{
			desc:     "First",
			request:  &types.QueryGetNodeAccountRequest{Index: msgs[0].Operator},
			response: &types.QueryNodeAccountResponse{NodeAccount: &msgs[0]},
		},
		{
			desc:     "Second",
			request:  &types.QueryGetNodeAccountRequest{Index: msgs[1].Operator},
			response: &types.QueryNodeAccountResponse{NodeAccount: &msgs[1]},
		},
		{
			desc:    "KeyNotFound",
