...
func TestChainNoncesQuerySingle(t *testing.T) {
	...
	chainNonces := sample.ChainNoncesList_pell(t, 2)
	...
	for _, tc := range []struct {
		...
		response *types.QueryChainNoncesResponse
		err      error
	}{
		{
			desc:     "First",
			request:  &types.QueryGetChainNoncesRequest{Index: chainNonces[0].Index},
			response: &types.QueryChainNoncesResponse{ChainNonces: chainNonces[0]},
		},
		{
			desc:     "Second",
			request:  &types.QueryGetChainNoncesRequest{Index: chainNonces[1].Index},
			response: &types.QueryChainNoncesResponse{ChainNonces: chainNonces[1]},
		},
		{
			desc:    "KeyNotFound",
...
func TestChainNoncesQueryPaginated(t *testing.T) {
	k, ctx, _, _ := keepertest.RelayerKeeper(t)
	wctx := sdk.WrapSDKContext(ctx)
	chainNonces := sample.ChainNoncesList_pell(t, 5)
	for _, nonce := range chainNonces {
		k.SetChainNonces(ctx, nonce)
	}
...
func TestPendingNoncesQuerySingle(t *testing.T) {
	...
	t.Run("should return if found", func(t *testing.T) {
		...
		tss := sample.Tss_pell()
		k.SetTSS(ctx, tss)
		pendingNonces := sample.PendingNoncesList_pell(t, "sample", 5)
