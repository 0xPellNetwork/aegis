...
// "cmp" todo: cmp.Ordered in version 1.21 of go
...
func mapHas[K int64, V any](m *map[K]V, key K) bool {
	_, ok := (*m)[key]
	return ok
}
...
func mapSet[K int64, V any](m *map[K]V, key K, value V, afterSet func(K, V)) {
	(*m)[key] = value
...
func mapUnset[K int64, V any](m *map[K]V, key K, beforeUnset func(K, V)) bool {
	if !mapHas(m, key) {
		return false
	}
...
func mapDeleteMissingKeys[K int64, V any](m *map[K]V, presentKeys []K, beforeUnset func(K, V)) {
