...
type VoterSuite struct {
	bridge1 *pellbridge.PellCoreBridge
	bridge2 *pellbridge.PellCoreBridge
}
...
func (s *VoterSuite) SetUpTest(c *C) {
	...
	chainHomeFoler := filepath.Join(homeDir, ".pellcored")
	c.Logf("chain home dir: %s", chainHomeFoler)

	// first signer & pellClient
	// alice is the default user created by Starport chain serve
	{
		...
		bridge, err := pellbridge.NewClient(k, chainIP, "alice")
		if err != nil {
			c.Fail()
		}
		s.bridge1 = bridge
	...
	// alice is the default user created by Starport chain serve
	{
		...
		bridge, err := pellbridge.NewClient(k, chainIP, "bob")
		if err != nil {
			c.Fail()
		}
		s.bridge2 = bridge
...
func (s *VoterSuite) TestGetBlockHeight(c *C) {
	h, err := s.bridge1.GetBlockHeight()
	c.Assert(err, IsNil)
	c.Logf("height %d", h)
}

func (s *VoterSuite) TestGetAccountNumberAndSeuqeuence(c *C) {
	an, as, err := s.bridge1.GetAccountNumberAndSequenceNumber()
	c.Assert(err, IsNil)
