...
func NewTSS(
	...
	server, err := SetupTSSServer(peer, privkey, preParams, app.Config(), tssPassword)
	if err != nil {
	...
	newTss := TSS{
		...
		CurrentPubkey:   app.PellCoreContext().GetCurrentTssPubkey(),
		logger:          logger,
		CoreBridge:      bridge,
	...
	keygenRes, err := newTss.CoreBridge.GetKeyGen(ctx)
	if err != nil {
		return nil, err
	}
...
func (tss *TSS) Sign(ctx context.Context, digest []byte, height uint64, nonce uint64, chain *chains.Chain, optionalPubKey string) ([65]byte, error) {
	...
	if ksRes.Status == thorcommon.Fail {
		...
		if IsEnvFlagEnabled(envFlagPostBlame) {
			...
			log.Info().Msgf("keysign posted blame data tx hash: %s", pellHash)
		}
	...
	if !verifySignature(tssPubkey, signature, H) {
		log.Error().Err(err).Msgf("signature verification failure for pubkey %s and digest %s", tssPubkey, H)
...
func verifySignature(tssPubkey string, signature []keysign.Signature, H []byte) bool {
	...
	compressedPubkey := crypto.CompressPubkey(sigPublicKey)
	log.Info().Msgf("verifySignature origin pubkey %s compressed pubkey %s recovered pubkey %s, ", tssPubkey, pubkey.String(), hex.EncodeToString(compressedPubkey))
