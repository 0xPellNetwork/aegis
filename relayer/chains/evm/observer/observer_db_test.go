...
func (suite *EVMClientTestSuite) TestEVMLastBlock() {
	...
	lastInboundBlockNum := uint64(12344)
	dbc := suite.db.Create(clienttypes.ToLastBlockSQLType(lastBlockNum, lastInboundBlockNum))
	suite.NoError(dbc.Error)
	...
	lastBlockNum++
	dbs := suite.db.Save(clienttypes.ToLastBlockSQLType(lastBlockNum, lastInboundBlockNum))
