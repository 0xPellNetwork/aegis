...
func init() {
	...
	InitCmd.Flags().StringVar(&initArgs.chainID, "chain-id", "ignite_7001-1", "chain id")
	InitCmd.Flags().StringVar(&initArgs.pellcoreURL, "pellcore-url", "127.0.0.1", "pellcore node URL")
	InitCmd.Flags().StringVar(&initArgs.authzGranter, "operator", "", "granter for the authorization , this should be operator address")
	InitCmd.Flags().StringVar(&initArgs.authzHotkey, "hotkey", "hotkey", "hotkey for pellclient this key is used for TSS and PellClient operations")
	...
	InitCmd.Flags().StringVar(&initArgs.TssPath, "tss-path", "~/.pellcored/.tss", "path to tss location")
...
func Initialize(_ *cobra.Command, _ []string) error {
	...
	configData.PellCoreURL = initArgs.pellcoreURL
	...
	//Save config file
	return config.Save(&configData, rootArgs.pellCoreHome)
}
