...
//const amount = "100000000" // Amount in apell
...
func main() {
	file, err := filepath.Abs(filepath.Join("cmd", "pellcore_utils", "address-list.json"))
	if err != nil {
		panic(err)
	}
	...
	fileS, err := filepath.Abs(filepath.Join("cmd", "pellcore_utils", "successful_address.json"))
	if err != nil {
		panic(err)
	}
	fileF, err := filepath.Abs(filepath.Join("cmd", "pellcore_utils", "failed_address.json"))
	if err != nil {
		panic(err)
	}
	...
	for i, address := range addresses {
		// #nosec G204
		cmd := exec.Command("pellcored", "q", "bank", "balances", address, "--output", "json", "--denom", "apell", "--node", node)
	...
	args = append(args, []string{distributionList[0].TokensDistributed.String(), "--keyring-backend", "test", "--chain-id", chainID, "--yes",
		"--broadcast-mode", broadcastMode, "--gas=auto", "--gas-adjustment=2", "--gas-prices=0.001apell", "--node", node}...)

	// #nosec G204
	cmd := exec.Command("pellcored", args...)
	output, err := cmd.CombinedOutput()
	if err != nil {
		fmt.Println(cmd.String())
		fmt.Println(fmt.Sprint(err) + ": " + string(output))
