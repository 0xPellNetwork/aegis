...
# cast call $ 'targetDelegations\(uint256\)\(bool\)' $OPERATOR_ADDRESS --rpc-url $PELL_EVM_URL
cast call $PELL_DELEGATION_MANAGER_INTERACTOR "targetDelegations(uint256)(address,uint256,uint256)" 97
...
if [ $IS_LOCALNET == "true" ]; then
  cast rpc evm_mine --rpc-url $EXTERNAL_RPC_URL
fi
